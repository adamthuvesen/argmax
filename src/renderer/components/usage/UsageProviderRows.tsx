...
/**
 * One tile per provider, and the page's filter: pressing a tile narrows the
 * hero, chart, totals, and breakdown to that provider; pressing it again
 * widens back out. The tiles themselves never narrow — their shares stay
 * shares of the whole window, which is what makes the comparison useful
 * while one provider is in focus.
 *
 * Rank is drawn as well as written: each tile is footed by a bar as long as
...
export function UsageProviderRows({
  ...
  return (
    <div className="usage-providers">
      {/* Named the way the figure above is, so the card reads as two parts:
          the total, then who it is made of. */}
      <p className="usage-eyebrow">By provider</p>
          ...
          const body = (
            <>
              {/* Rank is drawn as a rule along the tile's foot rather than as a
                  ...
                  as a *selected* tile, and for Codex — whose series colour is the
                  ...
                  CSS. Decorative: the percentage is in the text above it. */}
              {row.available ? (
                <svg
              ...
              <span className="usage-provider-body">
                {/* The share rides with the name rather than under the figure:
                    at a tile's width a percentage on the money line stole the
                    room a five-digit total needs, and "who, and how much of the
                    window" is one thought anyway. */}
                <span className="usage-provider-head">
                  ...
                  {row.available ? (
                    <span className="usage-provider-share">{formatPercent(share)}</span>
                  ) : null}
                </span>
