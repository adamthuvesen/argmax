...
The sidebar floats attention-worthy workspaces (session `attention` of `approval-needed`, `blocked`, `failed`, or `review-ready`; `archived`/`kept` excluded) into a Priority section directly under Pinned and above the date/project groups, sorted by severity then oldest-waiting. A row holds its place while the workspace is working and for 30 minutes after the session's last message (`PRIORITY_IDLE_MS`); a session quiet for longer than that — or of unknown age (pre-migration rows have no `attention_changed_at`) — is history, not triage, and stays out of the section. Opening a row does not demote it. The selector is pure and client-side: [src/renderer/lib/priority.ts](../src/renderer/lib/priority.ts) joins `snapshot.sessions` attention onto workspaces, so it updates via normal `dashboard:delta` merges. Because a row can go quiet with no delta to prompt a re-render, `computePriorityEntries` returns each entry's `idleAt` and the sidebar arms a single timer at `nextPriorityIdleAt` — one timeout for the exact moment the list changes, not a poll.

A workspace lives in exactly one sidebar section. Pinned takes precedence over Priority: a pinned workspace stays in Pinned even when it has fresh attention or a manual add. Unpinned Priority rows leave their date/project group and drop back when resolved, marked done, gone quiet, or pinned.
...
Right-click → "Done" calls `workspaces:set-priority-dismissed`, stamping `workspaces.priority_dismissed_at` (and clearing any manual add), which drops the row back into its date or project group depending on the sidebar's view mode. That is the only way to clear a row early: Priority is not an unread list, so reading a session leaves it in place, and a workspace that is still working never goes quiet however long the turn runs. A purely manual add (no attention) stays until explicitly removed. A dismissal only suppresses the row while it is newer than the session's `attention_changed_at` (stamped in `update_session_state` whenever the attention value changes), so a later wait after more agent work re-promotes the workspace without any server-side clearing. Right-click → "Add to priority" on an unpinned, non-priority row calls `workspaces:set-priority-added` (`workspaces.priority_added_at`): a manual add needs no attention, never goes quiet, and clears any standing dismissal. Pinned rows do not offer that action, because a pin already chooses their section. "Clear" on the Priority header is the bulk form of the dismissal: one `workspaces:set-priority-dismissed` per listed workspace, which empties both flavors of entry (dismissal clears a manual add) and drops the rows back into their date or project group. The section is toggleable in Settings → General (`argmax.sidebar.priority.visible`, default on).
