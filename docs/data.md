...
- `checkpoints` (v41) adds pinned worktree and index trees, HEAD, session boundaries, and recovery links to legacy checkpoint rows. `checkpoint_rewinds` journals each restore and its recovery checkpoint. Legacy rows remain readable but cannot restore files without tree snapshots.
