...
A turn that finishes cleanly exhales: one wavefront of accent pixels sweeps left→right along the bottom edge of the turn and dissolves, drawn on a canvas `TurnBlock` mounts for the ~1s it plays and drops on `onDone` ([TurnExhale.tsx](../src/renderer/components/TurnExhale.tsx)). It is the same field as the launcher composer's backdrop and the effort rail (`lib/pixelField.ts`), so the app has one pixel language rather than three.

- **The intensity is information.** `turnExhaleWeight` in [turnExhale.ts](../src/renderer/lib/turnExhale.ts) folds what the turn produced into one 0..1 number, and that number sets how far the front travels, how bright it burns, how long it runs, and whether the leading edge throws sparks. A one-line reply gets a short dim breath near the left edge; a turn that rewrote forty files crosses the whole card. So the size of what just happened is legible before a word of it is read. Written work dominates the weight, tool count is second (a turn that read thirty files and wrote nothing still did work), and prose length is capped low so a long-winded answer to a small question cannot outshine a quiet refactor. Files and lines take the larger of the two rather than stacking — they measure the same work at different resolutions.
- **It fires on the running → finished edge, seen by a mounted block.** `TurnBlock` seeds its `wasRunning` ref from the first render, so a reopened session — which remounts every turn with the work long over — exhales zero times instead of forty.
- **A turn that ended badly gets nothing.** The parent supplies the weight only when the session reached `complete`; `failed` and `cancelled` pass nothing at all. The gate is the session state rather than "no longer running" on purpose: a turn paused on a plan or a question also stops ticking, and breathing out halfway through one would say the opposite of what happened.
- **It paints behind the answer.** `.turn-block` takes `isolation: isolate` and the canvas sits at `z-index: -1`, so the field tints the page the prose sits on instead of washing over the glyphs, and no rule is needed on the turn's children. The band is capped at 132px and pinned to the bottom edge: a long answer runs thousands of pixels tall, a field behind all of it reads as a rendering fault, and the reader's eye at the moment a turn lands is at the bottom of it anyway.
- **Reduced motion, a hidden document, or no 2D context skip it** — and each still calls `onDone`, so the canvas is never left mounted. A hidden window advances no animation frames, so the sweep's own length also runs as a `setTimeout` deadline; without it a window backgrounded mid-sweep would keep the canvas forever.
...
- **The right dock, not a grid column:** Clicking the row opens the subagent in that session's review panel — a third mode beside Changes and Files ([AgentsView.tsx](../src/renderer/components/AgentsView.tsx)), so delegated work reads next to the work it came from. Each open subagent is a tab in the same strip Files mode uses; the transcript itself is [AgentActivity.tsx](../src/renderer/components/AgentActivity.tsx) and carries no chrome of its own. Every tab stays mounted, so a backgrounded subagent keeps polling. ⌘W closes the active tab, as it does for a file. The active tab's fixed metadata strip shows the reported model and effort (`Opus 5 · Extra High`), while the instructions line keeps only the agent role and whether it is still working remains the tab's own mark. A tab whose launch row leaves the timeline (a superseded Codex spawn) is dropped.
- **A subagent run is one turn, read at the chat's own verbosity.** The pane folds its tools with the same two passes the transcript uses (`foldConversationItems`, then `foldTurnToolItems`), so a run of calls reads as one group headline rather than a row per call, and wraps them in the same [TurnBlock.tsx](../src/renderer/components/TurnBlock.tsx): the `Worked for Xs` chip is one control over every tool group and Thought block below it. Where it starts comes from the chat-verbosity setting, not from a separate one — a pane that opened wide while the chat beside it read as single lines was two settings for one reader. The run is always its pane's latest turn, so it never collapses on age the way an older chat turn does. At Minimal the pane keeps only its result panel until the chip is opened, and unlike a chat turn it hides *all* pre-tool narration: the run's answer is the result panel, so there is no last prose group standing in for one.
