...
- **MCP servers:** Cursor reads the user and project `.cursor/mcp.json` files, and the OAuth grants it holds for them, when the ACP process starts. A session's callable tools are exactly the servers `cursor-agent mcp list` reports as `ready` at that path. Argmax fingerprints both config files and the set of servers with a stored grant in `~/.cursor/projects/<slug>/mcp-auth.json`, and replaces a workspace's warm process when the fingerprint changes, so adding a server or finishing `cursor-agent mcp login <name>` reaches the next chat instead of waiting for the process to die. Argmax's own temporary `argmax_*` PTY lease and token rotation inside a stored grant are excluded, because neither changes what Cursor can serve. Grants are per project path, so an isolated worktree still needs its own login. The per-session `argmax` server rides in `session/new` and `session/load` as an `mcpServers` entry, where the warm shared process receives its credential ([agent-tools.md](agent-tools.md)).
