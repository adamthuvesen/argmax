...
- **Standalone pages own the sidebar column.** Opening settings or schedule swaps `Sidebar` for a rail (`SettingsRail` / `ScheduleRail`) and stamps `data-settings-open` or `data-schedule-open` on `.app-shell`. The rail is a fixed `--settings-rail-width` and shows even when the app sidebar is collapsed. Settings' active group lives in `App`, not in the lazily-mounted panel, because the rail renders outside `Suspense`. Both pages share the content chrome in [settings-layout.css](../src/renderer/styles/settings-layout.css): empty `.settings-topbar` for drag, `.settings-page-title` in the column.
