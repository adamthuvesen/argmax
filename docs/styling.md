...
- **Font sizes flow through tokens.** Never hardcode a text `font-size` in px. There is one type scale in `tokens.css`, shifted whole pixels by `--type-step`: `[data-font-size="1"]`…`="10"` set the step (`-5px`…`+4px`), and the token block is declared on `:root, [data-font-size]` rather than `:root` alone, so any subtree carrying the attribute recomputes every size from its own step. Body text (`--text-base`) is `7 + level` px — 8px at level 1, the shipped 13px at the default 6, 17px at level 10. Two independent sizes exist, both on a 1–10 slider in Settings → Appearance:
  ...
  The session composer sits inside the grid but opts back out with
  `data-type-scale="chrome"` (`SessionComposer.tsx`): its model chip, repo and
  branch, changed-file counts and status line are chrome, and reading the
  transcript at 17px is no reason to inflate them. `--chrome-type-step` is
  captured on `:root` *after* the `[data-font-size]` ladder and is never
  rewritten below it, so a subtree can reach past any surrounding override for
  the app-chrome step. The result matches the launcher's composer, which is at
  chrome scale by virtue of living outside the grid. Both scales at the same
  level (the default) render identically, so this only shows once the two
  sliders diverge.
...
- **Motion.** Use `surface-in` / `fade-in` on mount, `msg-in` (and `msg-in-right` for user bubbles) on chat additions, `status-pulse` on running indicators, and `working-nest-relay` on the shared four-dot working nest ([WorkingNest.tsx](../src/renderer/components/WorkingNest.tsx), [working-nest.css](../src/renderer/styles/working-nest.css)). It is one mark for every live surface: the sidebar row's marker while a turn is in flight, a thinking line, a running agent launch row, an agent tab, and the agent pane header. Its dots transfer emphasis clockwise every 300 ms. A stable entity key keeps the same job synchronized across surfaces and offsets separate jobs by a quarter-cycle. Use `tool-call-flash` on new tool arrivals and `detail-expand` when a tool call row opens. New animations define a keyframe, reuse the motion tokens, and respect `prefers-reduced-motion`.
