...
pub async fn dashboard_list(
    ...
    dashboard_list_impl(&state).await
}
...
mod tests {
    ...
    #[tokio::test]
    async fn dashboard_list_reports_why_the_database_never_opened() {
        ...
        let error = dashboard_list_impl(&state)
            .await
            .expect_err("no database means no snapshot");
    ...
    #[tokio::test]
    async fn dashboard_list_says_boot_is_in_flight_when_nothing_failed() {
        let error = dashboard_list_impl(&AppState::new())
            .await
            .expect_err("no database means no snapshot");
