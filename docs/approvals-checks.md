...
A tool call the chat draws as an **interactive card** is never gated. The card is already in front of the user with its own button. Claude's question card terminates the turn and sends the answer as a new message, while synchronous Codex questions resolve the waiting request. An extra Approve/Reject row would ask permission to show a question the user is looking at. `renders_as_interactive_card` in [providers/mod.rs](../src-tauri/src/providers/mod.rs) holds the names, in step with [turnInteractiveCards.ts](../src/renderer/lib/turnInteractiveCards.ts).

Claude's question tool is answered with a **deny whose message states that contract**, not an allow. `AskUserQuestion` reads its answers from the permission response, and an allow without any makes the CLI report "The user did not answer the questions." before the user has seen the dock; the model reads that as a dismissal and re-asks in prose or retries the tool. The deny message tells it the card is on screen, the answer arrives as the next user message, and the turn should end now (`card_shown_message` in [claude_control.rs](../src-tauri/src/providers/claude_control.rs)). `SendUserMessage` is prose, not a wait, and stays allowed. The card is drawn from the call's input, so the result text never reaches the chat.
