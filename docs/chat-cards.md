...
- **Running work is marked on the words.** The Thinking word, a running group's headline and a running row's verb and target carry the reading wave ([reading-wave.css](../src/renderer/styles/reading-wave.css), [readingWave.ts](../src/renderer/lib/readingWave.ts)): a soft band of ink tinted with the accent reads the line left to right at a constant 11.05em/s and rests 0.7s between passes. The iPhone draws the same band ([ReadingWave.swift](../ios/Argmax/Sources/Design/ReadingWave.swift)); the tuning is in [docs/design/live-text-motion](design/live-text-motion/README.md). The phase comes from the wall clock, so a remount or a headline change never restarts the pass, and elapsed seconds stay still. Under Reduce Motion the words are plain text.
