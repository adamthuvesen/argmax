...
- **Server** ([src-tauri/src/remote](../src-tauri/src/remote)): axum on
  ...
  Content-hashed files under `/assets/` are served `immutable`; the shell keeps
  its URL across builds, so it is served `no-cache` with a content-derived
...
- **Protocol**: JSON text frames. First frame must be
  ...
  never surfaces protocol-level pongs to JS. A client that falls behind the
  event stream gets `{"type":"resync"}` instead of the dropped frames: the
  socket stays open, so this is the only signal that its snapshot is stale
  and it must refetch.
- **Dispatcher** ([dispatch.rs](../src-tauri/src/remote/dispatch.rs)): a match
  ...
  Affordances that call one of those channels must branch on `isRemoteBridge()`
  ([tauriBridge.ts](../src/renderer/lib/tauriBridge.ts)) rather than fire a
  request that can only fail: shared components render on the phone too, and
  `REMOTE_UNSUPPORTED` reaches them as a raw Error. Two do so today — a web
  link in chat keeps the anchor's own `target="_blank"` instead of routing
  through `system:open-path` (which would open it on the host, and whose
  swallowed rejection left the tap doing nothing at all), and pasting an image
  into the composer answers "Attaching images needs the desktop app."
...
- **Mobile entry** ([mobile.html](../mobile.html),
  ...
  as the desktop sidebar), a per-row actions sheet (pin, rename, archive), and a
  ...
  Desktop-only chrome is hidden by `styles/mobile.css`, not ported, and the
  shell hosts its own type scale (`data-font-size="8"`) because a phone is read
  at arm's length.

  Screen depth is mirrored into browser history
  ([useMobileBackNavigation.ts](../src/renderer/mobile/useMobileBackNavigation.ts)),
  so Android's back button and iOS's back-swipe pop one screen instead of
  leaving the app — from an installed PWA, leaving means killing the view.
  In-app closes rewind history by the same amount and those self-inflicted
  pops are counted and ignored, keeping the two stacks the same length.

  The page is installable: [public/manifest.webmanifest](../public/manifest.webmanifest)
  carries the icon `public/argmax-icon.png` (generated by
  [scripts/build-icons.mjs](../scripts/build-icons.mjs) — unhashed, because
  Vite would hash anything referenced from HTML and the copied manifest would
  then 404). `server.rs` serves `.webmanifest` as `application/manifest+json`;
  as `application/octet-stream` Chrome drops the manifest silently.
...
Each push carries a `Click` header of `<mobile page>?session=<id>`, so tapping
it opens the transcript that raised it rather than the session list. The phone
reads that param once and scrubs it
([deepLink.ts](../src/renderer/mobile/deepLink.ts)), so a later reload lands on
the list. The base URL is `mobile_url` in `remote.json`, re-derived from the
tailnet probe every time Settings saves — the publisher runs far from that
async probe and needs the value at boot too. It is the same
`mobile_page_url()` the pairing QR encodes, so a notification can never point
somewhere the phone was not paired; if the tailnet name changes, saving
Settings again refreshes it. A push sent before any save carries no `Click`
header rather than a guessed URL.
