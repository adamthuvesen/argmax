...
- **Single calls respect the verbosity.** Minimal and Compact keep even a single call behind a short summary such as "Ran a command", so a long command cannot dominate the conversation. Steps and Detailed show a single call as its target row.
