...
- **Shared values, not duplicates.** Model labels/ids/reasoning/launch mode live in [src/shared/providerModels.ts](src/shared/providerModels.ts) — `PROVIDER_MODEL_DEFAULTS` is the single source of truth. Current defaults: Claude Haiku 4.5 (structured JSON), Codex Spark medium reasoning (structured JSON). Renderer ships a model picker; defaults are the launch fallback.
