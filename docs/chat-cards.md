...
- **Suggested follow-up:** once a turn completes, [useFollowUpSuggestion.ts](../src/renderer/hooks/useFollowUpSuggestion.ts) asks `session:suggest-follow-up` for the reply the user would most plausibly send, and shows it as the composer placeholder instead of the static hint. One call per completed turn (keyed on `completedAt`), on the cheap title model; a failure or an empty answer keeps the static placeholder.
