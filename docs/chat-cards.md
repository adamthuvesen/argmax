...
- **Live streaming:** the block that owns the beat renders expanded and labelled "Thinking". Settled inline reasoning in Balanced and Detailed shows only the reasoning text, without a "Thought" heading or duration. Inline reasoning uses the normal message text color. Collapsible reasoning keeps its muted color.
