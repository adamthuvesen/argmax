...
- `projects.archive_on_merge` (v31) is the per-project opt-in that lets the gh poller archive a workspace once the PR on its branch merges. `0`/`1`, defaulting to off, because archiving removes a worktree and its local branch. See [gh.md](gh.md).
