...
- **Minimal:** activity summaries and the live caption while working. Finished turns hide all tool activity, including failed attempts, and pre-tool narration behind `Worked for`, keeping the answer, changed files, and session-level errors visible. The chip counts what it hides ("Worked for 1m 26s · 11 steps"). Opening the chip restores narration and activity summaries. A failed attempt does not keep its group in the finished transcript, since agents commonly retry and succeed.
