...
- **Font sizes flow through tokens.** Never hardcode a text `font-size` in px. There is one type scale in `tokens.css`, shifted whole pixels by `--type-step`: `[data-font-size="1"]`…`="10"` set the step (`-3px`…`+6px`), and the token block is declared on `:root, [data-font-size]` rather than `:root` alone, so any subtree carrying the attribute recomputes every size from its own step. Body text (`--text-base`) is `9 + level` px — 10px at level 1, the shipped 15px at the default 6, 19px at level 10. Two independent sizes exist, both on a 1–10 slider in Settings → Appearance:
  - **App font size** flips `<html data-font-size="…">` and covers app chrome: sidebar, titlebar, settings, the full-screen launcher, and global overlays such as the command palette. Persisted under `argmax.font.scale`.
  - **Agent window font size** flips `data-font-size` on the session grid root (`.session-multigrid` in `SessionMultiGrid.tsx`), so conversations, tool and thinking cards, and agent activity panes re-resolve the type tokens on their own. Persisted under `argmax.font.scale.chat`, and an install with no chat key inherits the app value so nothing jumps on upgrade.

  The session composer, the workspace card, and the review panel sit inside
  the grid but opt back out with `data-type-scale="chrome"`
  (`SessionComposer.tsx`, `WorkspaceCard.tsx`, `ReviewPanel.tsx`): the
  composer's model chip, repo and branch, changed-file counts and status line
  are chrome; the workspace card is sidebar-class chrome — branch, repo, PR
  actions; and the review panel holds files, changes, and the diff code
  between them. All three match the left sidebar's scale — the transcript
  sitting at 19px is no reason to inflate them.
  `--chrome-type-step` is captured on `:root` *after* the `[data-font-size]`
  ladder and is never rewritten below it, so a subtree can reach past any
  surrounding override for the app-chrome step. The result matches the
  launcher's composer, which is at chrome scale by virtue of living outside the
  grid. Both scales at the same level (the default) render identically, so this
  only shows once the two sliders diverge.

  The font sizes live in [fonts.ts](../src/renderer/lib/fonts.ts); chat width keeps the 1–5 scale from [scaleLevel.ts](../src/renderer/lib/scaleLevel.ts). A value under a retired key — the 1–5 scale's `argmax.font.size` / `argmax.font.size.chat` (old level N reads as N + 3), or the pre-scale ids (`small` / `large`, `narrow` / `wide`) — migrates on read, and the next persist writes the current level under the current key.

  Neither size has a keyboard shortcut. Settings is the only entry point. Terminal/xterm surfaces must resolve `--text-terminal` through the helpers in `src/renderer/lib/fonts.ts`, because canvas text cannot inherit CSS variables directly. That resolution reads `documentElement`, so the terminal keeps its own size and follows app chrome rather than the agent-window setting. `--text-terminal` is registered via `@property` as a `<length>` in `tokens.css`, and that registration is load-bearing: an unregistered custom property is substituted rather than computed, so reading it back from JS yields the literal `calc(…)` string instead of pixels and the terminal silently pins to the parse fallback at every level. Any other type token that grows a JS reader needs the same registration.
...
- **Motion.** Use `surface-in` / `fade-in` on mount, `msg-in` (and `msg-in-right` for user bubbles) on chat additions, `status-pulse` on running indicators, and `working-nest-relay` on the shared four-dot working nest ([WorkingNest.tsx](../src/renderer/components/WorkingNest.tsx), [working-nest.css](../src/renderer/styles/working-nest.css)). It is the app's only loading indicator — there are no rotating spinners — and it marks every live surface: the sidebar row's marker while a turn is in flight, a thinking line, a running agent launch row, an agent tab, the agent pane header, running tool rows and their group headers, the workspace card's subagents section, a loading browser tab, the command palette's in-flight searches, and provider-discovery refresh buttons while they run. Its dots transfer emphasis clockwise every 300 ms. A stable entity key keeps the same job synchronized across surfaces and offsets separate jobs by a quarter-cycle. Use `detail-expand` when a tool call row opens. New animations define a keyframe, reuse the motion tokens, and respect `prefers-reduced-motion`.
...
User message bubbles (`chat-bubble.user`) use `--user-message-bg` / `--user-message-fg` / `--user-message-shadow`. Both themes fill the bubble with the active accent (`--accent`); `--user-message-fg` follows `--bubble-on-ink`, so light themes pair mid-tone accents with white ink and dark pairs its lifted pastel accents with ink. The `/skill` chip and any markdown links or inline code inside the bubble ride the fg token too, since their default accent-deep ink disappears on an accent background.

`::selection` uses a translucent accent tint globally. Inside user bubbles, `.chat-bubble.user ::selection` uses `--user-message-selection-bg`, which tints toward the bubble fg so the highlight stays visible on the accent surface in either theme.

A user bubble spans the full conversation column when its text needs it — there is no left gutter, since the accent slab already separates it from the agent's prose. Its body is clipped at six lines (`.chat-bubble-body`, faded at the cut) and the remainder opens in place through the bare-text Show more / Show less toggle in the bubble's bottom-left corner ([ChatBubble.tsx](../src/renderer/components/ChatBubble.tsx)). The clip is not a scrollport on purpose: a long paste must not push the reply off-screen, and it must not trap the wheel either.
