...
- **Minimal:** activity summaries alone while working — no live caption, and the summary's chevron held back until hover or focus. The line reports that work is happening, not what is running, and the turn chip is the standing disclosure. Clicking the headline still opens it. Finished turns hide all tool activity, including failed attempts, and pre-tool narration behind `Worked for`, keeping the answer, changed files, and session-level errors visible. The chip counts what it hides ("Worked for 1m 26s · 11 steps"). Opening the chip restores narration and activity summaries. A failed attempt does not keep its group in the finished transcript, since agents commonly retry and succeed.
