...
The question tool is answered with a **deny whose message states that contract**, not an allow. Claude's `AskUserQuestion` reads its answers from the permission response, and an allow without any makes the CLI report "The user did not answer the questions." before the user has seen the dock; the model reads that as a dismissal and re-asks in prose or retries the tool. The deny message tells it the question is on screen, the answer arrives as the next user message, and the turn should end now (`question_shown_response` in [claude_control.rs](../src-tauri/src/providers/claude_control.rs)). The card is drawn from the call's input, so the result text never reaches the chat.
