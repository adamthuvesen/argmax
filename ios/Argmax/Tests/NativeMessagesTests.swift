...
final class NativeMessageDecodingTests: XCTestCase {
    ...
    func testDecodesEveryMessageType() {
        ...
        XCTAssertEqual(NativeMessage(body: ["type": "question", "open": false]), .question(open: false))
...
final class TranscriptHostQueueTests: XCTestCase {
    ...
    func testPublishesWhatThePageReports() {
        ...
        host.receive(.question(open: true))
        XCTAssertTrue(host.questionOpen, "the composer card stands down while the question panel has its slot")
