...
- **The writing head fades in.** Each tick's new characters are a *fresh run* ([streamFreshRuns.ts](../src/renderer/lib/streamFreshRuns.ts)): the reveal records where a run started and when, a rehype plugin cuts the rendered tree at those offsets, and each run fades from 30% opacity to its own ink over 420 ms. Six or so runs are in flight at once, so the writing head carries a soft left-to-right gradient instead of stamping a block of characters down at full ink, and the last runs finish after the stream has ended rather than snapping. Only the typewriter's own output fades: a live thought is unpaced, a tool row is not markdown, and code blocks are skipped because the highlighter owns their reveal. The fade is opacity and nothing else, so it composites; its phase is anchored to the wall clock rather than to the span, because re-parsing the markdown every tick hands a run's text to whichever span now sits in that position. Under reduced motion there is no typewriter, so there is no fade either.
