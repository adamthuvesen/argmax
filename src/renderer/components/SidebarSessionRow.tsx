...
function StatusMarker({
  ...
  if (priorityAttention === "approval-needed" || priorityAttention === "blocked") {
    return <CircleEllipsis size={18} aria-hidden className="status-marker" data-attention={priorityAttention} />;
  }
  if (state === "running") {
    // The shared working nest, sized to the 18px marker box. It is the same mark and
    // motion an agent tab and a sub-agent launch row show while they run.
    return <WorkingNest active className="status-marker" size={18} phaseKey={phaseKey} />;
  }
  if (prState === "MERGED") {
    return <GitMerge size={18} aria-hidden className="status-marker" data-pr="merged" />;
  }
  if (prState === "OPEN") {
    return <GitPullRequest size={18} aria-hidden className="status-marker" data-pr="open" />;
  }
  return <CircleX size={18} aria-hidden className="status-marker" />;
}
