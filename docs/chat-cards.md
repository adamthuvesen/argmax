...
- **Activity icons have colour on desktop and iPhone.** Icons stay static, with no wobble, rotation, or pulsing. Blue marks reads, commands, and computer use. File changes use amber, search/list/web teal, and images/discovery/skills violet. MCP rows retain their integration artwork, and shell rows can use personal command icon rules described below. The first activity supplies the desktop group icon. iPhone groups show up to three distinct activity or integration icons, or one in Minimal. Labels distinguish running, succeeded, failed, and cancelled operations without using failure colour or automatically expanding a row. Unconfirmed completions use the neutral activity label instead of exposing transport bookkeeping.
...
- **Codex summary boundaries:** the app-server translator separates streamed reasoning parts and does not replay a completed item already streamed. Historical items whose summary parts were concatenated without separators are corrected during desktop replay when the completed summary exactly matches that saved suffix.
...
- **Opening Agents discovers the session's work.** Opening the right panel with ⌘B and choosing Agents populates tabs from the same subagent and multitask roster as the workspace card. The strip puts everything still running first and, within each group, the newest launch leftmost, so what is active shows without scrolling once many are open. New arrivals appear without changing the selected tab. Closing a tab keeps it closed during that visit, and reopening the Agents view discovers the available work again.
