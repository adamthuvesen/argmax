...
- **All IPC** flows through `window.argmax.*`. Request/response channels use schemas in [src/shared/ipcSchemas.ts](src/shared/ipcSchemas.ts) and `withValidation()` in [src/main/ipc.ts](src/main/ipc.ts). `IPC_CHANNELS` is derived from `Object.keys(ipcSchemas)` and is enforced by a regression test — adding a request/response channel is a five-step dance ([agents/docs/ipc.md](agents/docs/ipc.md#adding-a-requestresponse-channel)). Push-only event channels (`dashboard:delta`, `terminal:data`, `terminal:exit`, `menu:command`) are typed in preload but do **not** belong in `IPC_CHANNELS`.
...
- **Light theme is non-negotiable.** No dark mode. Font family is user-pickable from Settings → Appearance (Lilex is the default; alternates load via `@fontsource`). See [agents/docs/styling.md](agents/docs/styling.md).
