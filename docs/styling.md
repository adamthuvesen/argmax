...
- **Three themes: Light / Dark / System.** Dark is the default. System tracks `prefers-color-scheme` live when chosen. Dark is **warm charcoal**: yellow-leaning grays (`oklch(20% 0.006 107)` family), never cool/midnight blue and never near-black. In dark, `--bg` is the darkest well (main view), `--sidebar` sits a step above it, and `--panel` is the elevated card surface. Tokens live in `:root` (light) and `:root[data-theme="dark"]`. Active mode persists under `argmax.theme.mode` localStorage and `userData/theme.json` (Tauri-side for no-flash startup). See [src/renderer/lib/theme.ts](../src/renderer/lib/theme.ts).
...
- **Font sizes flow through tokens.** Never hardcode a text `font-size` in px. The type scale is declared by plain `[data-font-size="small|default|large"]` rules in `tokens.css`, not `:root`-anchored ones, so a scale applies to whatever subtree carries the attribute. Two independent sizes exist, both Small / Default / Large in Settings → Appearance:
...
Future text-size changes should update the `data-font-size` mode blocks, not individual component rules. Each mode block lists the full scale, so a nested container that sets the attribute overrides every type token for its subtree. Reduced-motion users get a zero override on the motion tokens via `@media (prefers-reduced-motion: reduce)`.
...
- **Status edges.** A running `.tool-call-item` gets one amber left `inset box-shadow`; an errored one gets rose. The icon signals the tool *type*, while color signals state. `data-tool-type` (`bash | edit | read | search | web | other`, set by `getToolTypeBucket()` in `App.tsx`) is still carried for the icon and the agent-row treatment (`.tool-call-row[data-tool-type="agent"]`), but no longer drives a per-type accent bar.
- **Motion.** Use `surface-in` / `fade-in` on mount, `msg-in` (and `msg-in-right` for user bubbles) on chat additions, `status-pulse` on running indicators, `status-marker-working-pulse` on the sidebar row's working marker while a turn is in flight (four nest dots staggered a quarter-cycle apart, or the custom-icon overlay dot on its own), `agent-launch-nest-orbit` on the four-dot nest mark of a running agent launch row, `tool-call-flash` on new tool arrivals, `detail-expand` when a tool call row opens, and the thinking indicator pulse/typing affordances. New animations: define a keyframe, reuse `--ease`, and respect the `prefers-reduced-motion` block at the bottom of the file.
...
6. **Ink stops short of paper-white.** `--text` is `#e3e0d8`, not `#f4f2ec`: full white on warm charcoal blooms and reads bold even at regular weight. Pair it with the lowered `--weight-ui` / `--weight-prose` rather than reaching for a brighter hex.
