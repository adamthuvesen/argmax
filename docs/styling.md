...
- **Motion.** Use `surface-in` / `fade-in` on mount, `msg-in` (and `msg-in-right` for user bubbles) on chat additions, `status-pulse` on running indicators, `status-marker-working-pulse` on the sidebar row's working marker while a turn is in flight, `agent-launch-nest-orbit` on the four-dot nest mark of a running agent launch row, `tool-call-flash` on new tool arrivals, `detail-expand` when a tool call row opens, and the thinking indicator pulse/typing affordances. New animations: define a keyframe, reuse `--ease`, and respect the `prefers-reduced-motion` block at the bottom of the file.
