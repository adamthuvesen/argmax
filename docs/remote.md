...
The `--http=` above is what makes the phone an insecure origin, and that costs
more than it looks: `crypto.randomUUID` and `navigator.clipboard` are missing
there (see **Insecure origin** below), and a service worker cannot register at
all, so the app shell never caches. Dropping the flag makes Serve terminate TLS
with a real tailnet certificate:

```bash
tailscale serve --bg 8790
```

Two things to know before switching. HTTPS and MagicDNS have to be enabled for
the tailnet first — with certificates off, plain `tailscale serve` hangs rather
than reporting the problem. And `https://` is a **new origin**: the home-screen
icon has to be removed and re-added, the phone re-paired, and the appearance
preferences kept in `localStorage` (`argmax.theme.mode`, `argmax.accent.tint`,
`argmax.chat.bubbleTint`) start again at their defaults.
...
[src-tauri/src/remote/ntfy.rs](../src-tauri/src/remote/ntfy.rs) watches session state changes and sends HTTP POST notifications for events requiring attention (`approval-needed`, `question-asked`, `blocked`, `failed`, `complete`). Each notification includes a click action URL linking directly to the session.
